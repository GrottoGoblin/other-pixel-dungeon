...
import com.grottogoblin.otherpixeldungeon.actors.hero.HeroClass;
...
import com.grottogoblin.otherpixeldungeon.ui.ExitButton;
import com.grottogoblin.otherpixeldungeon.ui.IconButton;
import com.grottogoblin.otherpixeldungeon.ui.Icons;
import com.grottogoblin.otherpixeldungeon.ui.RenderedTextBlock;
...
import com.grottogoblin.otherpixeldungeon.windows.WndChallenges;
import com.grottogoblin.otherpixeldungeon.windows.WndHeroInfo;
...
import com.grottogoblin.otherpixeldungeon.windows.WndTextInput;
...
public class HeroSelectScene extends PixelScene {
	...
	public void create() {
		...
		infoButton = new IconButton(Icons.get(Icons.INFO)){
			...
			protected void onClick() {
				...
				otherpixeldungeon.scene().addToFront(w);
			}
	...
	protected void onBackPressed() {
		if (btnExit.active){
			otherpixeldungeon.switchScene(TitleScene.class);
		} else {
			super.onBackPressed();
		}
	}
	...
	private class HeroBtn extends StyledButton {
		...
		protected void onClick() {
			...
			if( !cl.isUnlocked() ){
				otherpixeldungeon.scene().addToFront( new WndMessage(cl.unlockMsg()));
			...
			if (DeviceCompat.isDebug() || Badges.isUnlocked(Badges.Badge.VICTORY)){
				StyledButton seedButton = new StyledButton(Chrome.Type.BLANK, Messages.get(HeroSelectScene.class, "custom_seed"), 6){
					...
					protected void onClick() {
						...
						otherpixeldungeon.scene().addToFront( new WndTextInput(Messages.get(HeroSelectScene.class, "custom_seed_title"),
								...
								if (positive && seed != -1){
									...
									for (GamesInProgress.Info info : GamesInProgress.checkAll()){
										if (info.customSeed.isEmpty() && info.seed == seed){
											SPDSettings.customSeed("");
											icon.resetColor();
											otherpixeldungeon.scene().addToFront(new WndMessage(Messages.get(HeroSelectScene.class, "custom_seed_duplicate")));
											return;
										}
									}
				...
				StyledButton dailyButton = new StyledButton(Chrome.Type.BLANK, Messages.get(HeroSelectScene.class, "daily"), 6){
					...
					protected void onClick() {
						...
						if (diff > 24*HOUR){
							otherpixeldungeon.scene().addToFront(new WndMessage(Messages.get(HeroSelectScene.class, "daily_unavailable_long", (diff / DAY)+1)));
							return;
						}

						for (GamesInProgress.Info game : GamesInProgress.checkAll()){
							if (game.daily){
								otherpixeldungeon.scene().addToFront(new WndMessage(Messages.get(HeroSelectScene.class, "daily_existing")));
								return;
							}
						}
						...
						otherpixeldungeon.scene().addToFront(new WndOptions(
								icon,
				...
				StyledButton challengeButton = new StyledButton(Chrome.Type.BLANK, Messages.get(WndChallenges.class, "title"), 6){
					@Override
					protected void onClick() {
						otherpixeldungeon.scene().addToFront(new WndChallenges(SPDSettings.challenges(), true) {
